...
4. **Pick up tasks**: Read `shared/task_list.json`, find frontend testing tasks assigned to you, set status to `in_progress`.
5. **Check dependencies**: Before writing tests for a component, check if the frontend task is `done`. If not, set your task to `blocked` with `blocked_by`.
