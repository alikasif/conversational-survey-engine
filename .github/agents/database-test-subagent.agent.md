...
4. **Pick up tasks**: Read `shared/task_list.json`, find database testing tasks assigned to you, set status to `in_progress`.
5. **Check dependencies**: Before testing migrations, check if the database tasks are `done`. If not, set your task to `blocked` with `blocked_by`.
